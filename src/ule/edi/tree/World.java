...
/**
 * Un mundo es un árbol binario. En cada nodo de un mundo se almacena una lista
 * de entidades, cada una con su tipo y cardinalidad. Ver {@link Entity}.
 * 
 * Si se codifica "bajar por la izquierda" como "0" y "bajar por la derecha"
 * como "1", el camino desde un nodo N hasta un nodo M (en uno de sus
 * sub-árboles) será la cadena de 0s y 1s que indica cómo llegar desde N hasta
 * M.
 *
 ...
 * ∅
 * 
 * contiene un bosque (forest) en "", otro en "0", dos dragones y una princesa
 * en "00" y un castillo en "01".
 * 
 * @param <T>
 * 
 */
public class World extends AbstractBinaryTreeADT<LinkedList<Entity>> {
	 ...
	 * 
	 * La inserción se produce en el nodo indicado por la dirección; todos los nodos
	 * recorridos para alcanzar aquél que no estén creados se inicializarán con una
	 * entidad 'forest'.
	 * 
	 * La dirección se supondrá correcta, compuesta de cero o más 0s y 1s.
	 * 
	 * Dentro de la lista del nodo indicado, la inserción de nuevas entidades se
	 * realizará al final, como último elemento.
	 * 
	 * Por ejemplo, en un árbol vacío se pide insertar un 'dragón' en la dirección
	 * "00". El resultado final será:
	 * 
	 * [F(1)] | [F(1)] | | [D(1)] | | | ∅ | | | ∅ | | ∅ | ∅
	 * 
	 * La dirección "" indica la raíz, de forma que insertar un 'guerrero' en "" en
	 * el árbol anterior genera:
	 * 
	 * [F(1), W(1)] | [F(1)] | | [D(1)] | | | ∅ | | | ∅ | | ∅ | ∅
	 * 
	...
	/**
	 * Calcula el número de princesas accesibles que hay en este mundo situadas en
	 * la altura h, e introduce en una lista las referencias a los nodos en las que
	 * se encuentran.
	 * 
	 * Una princesa es accesible si en el camino desde la raiz hasta ella no aparece
	 * ningún Dragón
	 * 
	 * @param List<World> donde dejará las referencias a los nodos situados en
	 *        altura h que contienen princesas accesibles.
